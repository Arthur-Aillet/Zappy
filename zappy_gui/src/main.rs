...
use crate::tantorian::{generate_color_from_string, PlayerState, Tantorian};
...
fn zappy_app(nannou_app: &rend_ox::nannou::App) -> App<Zappy> {
    ...
    app.user.teams.push(("Team5".to_string(), generate_color_from_string(&"Team5".to_owned())));
    let mut new = Tantorian::new_from_command(
        1, 0, 0,
        crate::tantorian::Orientation::S, 1, "Team5".to_string(),
        &app.user.teams, &app.user.map.size, &mut app.user.players).unwrap();
    new.state = PlayerState::Egg;
