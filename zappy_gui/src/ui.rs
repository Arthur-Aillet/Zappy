...
impl ZappyUi {
    ...
    pub(crate) fn win(&mut self, ctx: &CtxRef, team_name: &String, is_active: bool) {
        egui::Window::new("Winner team found!")
            .enabled(!is_active)
            .show(ctx, |ui| {
    ...
    pub(crate) fn settings(
        &mut self,
        ctx: &CtxRef,
        camera: &mut rend_ox::camera::Camera,
        is_active: bool,
    ) -> Option<i32> {
        ...
        egui::Window::new("Settings")
            ...
            .show(ctx, |ui| {
                ...
                ui.label("Game:");
                ui.add(
                    egui::Slider::new(&mut self.selected_time_unit, 1..=10000)
                        .text("Time unit:")
                        .logarithmic(true),
                );
    ...
    pub(crate) fn network_status(
        ...
        egui::Window::new("Network Status")
            ...
            .show(ctx, |ui| {
                ...
                if status {
                    ui.add(egui::Label::new("Current status: Connected".to_string()));
                } else {
                    ui.add(egui::Label::new("Current status: Disconnected".to_string()));
                }
                ...
                ui.add(
                    egui::Label::new("________________________________________________").strong(),
                );
                ui.add_space(10.);
                        ...
                        egui::Grid::new("Network Messages")
                            ...
                            .show(ui, |grid| {
                                for message in &self.network_messages {
                                    ZappyUi::display_stat(
                                        grid,
                                        &format!("<{:.4}s>", message.0.as_secs_f32()),
                                        &message.1,
                                    );
                                }
                            });
                ...
                egui::Grid::new("Broadcast Messages")
                    ...
                    .show(ui, |grid| {
                        for (time, team_name, number, message) in self.broadcast_messages.iter() {
                            ZappyUi::display_stat(
                                grid,
                                &format!("<{team_name}:{number} {:.1}s>", time.as_secs_f32()),
                                message,
                            );
                        }
                    });
        ...
        for x in 0..map.size[0] {
            for y in 0..map.size[1] {
                ...
                if let Some(tile) = self.selected_tile {
                    clicked_last_frame = !has_been_clicked && tile[0] == x && tile[1] == y
                }
    ...
    fn tile_content(
        ...
        ZappyUi::display_stat(
            grid,
            "Linemate",
            map.tiles[selected[0]][selected[1]].q1.len(),
        );
        ZappyUi::display_stat(
            grid,
            "Deraumere",
            map.tiles[selected[0]][selected[1]].q2.len(),
        );
        ZappyUi::display_stat(grid, "Sibur", map.tiles[selected[0]][selected[1]].q3.len());
        ZappyUi::display_stat(
            grid,
            "Mendiane",
            map.tiles[selected[0]][selected[1]].q4.len(),
        );
        ZappyUi::display_stat(grid, "Phiras", map.tiles[selected[0]][selected[1]].q5.len());
        ZappyUi::display_stat(
            grid,
            "Thystame",
            map.tiles[selected[0]][selected[1]].q6.len(),
        );
        ...
        for player in players.values() {
            if &player.current_tile.as_uvec2().as_ref().map(|x| x as usize) == selected
                && player.state == Alive
            {
        ...
        egui::CollapsingHeader::new("Settings")
            ...
            .show(ui, |col| {
                egui::Grid::new("Settings")
                    ...
                    .striped(true)
                    .show(col, |grid| {
                        grid.add(egui::Label::new("Refresh:".to_string()).underline());
                        state = grid.add(egui::Button::new("Refresh")).clicked();
                        grid.end_row();
                        grid.add(egui::Label::new("Auto refresh:".to_string()).underline());
                        grid.add(egui::Checkbox::new(auto_refresh, ""));
                        grid.end_row();
                        if *auto_refresh {
                            grid.add(egui::Label::new("Refresh rate:".to_string()).underline());
                            grid.add(
                                egui::Slider::new(refresh_rate, 1.0..=1000.0).logarithmic(true),
                            );
                            grid.end_row();
                        }
                    });
                    ...
                    egui::Grid::new("Player data")
                        ...
                        .show(col, |ui_grid| {
                            ...
                            ZappyUi::display_stat(
                                ui_grid,
                                "Position",
                                format!("{} {}", player.current_tile.x, player.current_tile.y),
                            );
                            ZappyUi::display_stat(ui_grid, "Level", format!("{}", player.level));
                            ZappyUi::display_stat(
                                ui_grid,
                                "Orientation",
                                player.orientation.as_char(),
                            );

                            ui_grid.add(egui::Label::new("Color:".to_string()).underline());
            ...
            .show(ctx, |ui| {
                ...
                for (team_index, team) in teams.iter_mut().enumerate() {
                    egui::CollapsingHeader::new(&team.0)
                        .default_open(false)
                        .show(ui, |col| {
                            col.add(egui::Label::new("Color:".to_string()).underline());
        ...
        if let Some(team_index) = what_team_color_regenerate {
            for player in players.values_mut() {
                if player.team_name == teams[team_index].0 {
                    player.color = teams[team_index].1.lerp(
                        generate_color_from_string(&format!("{}", player.number)),
                        0.2,
                    );
                }
            }
