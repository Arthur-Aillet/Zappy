...
impl Zappy {
    ...
    pub(crate) fn close_connection(&mut self, at: Duration) {
        self.ui.network_messages.push((
            at,
            format!("Closing connection to {}:{}", self.hostname, self.port),
        ));
    ...
    pub(crate) fn try_to_connect(&mut self, at: Duration) {
        ...
        self.ui.network_messages.push((
            at,
            format!("New connection tried to {}:{}", self.hostname, self.port),
        ));
        ...
        if let Some(server) = &mut self.server {
            ...
            self.ui.network_messages.push((
                at,
                format!("Connection to {}:{} working", self.hostname, self.port),
            ));
        } else {
            self.ui.network_messages.push((
                at,
                format!("Connection to {}:{} failed", self.hostname, self.port),
            ));
        }
    }
}
...
pub(crate) fn loop_server(mut server: ServerConn) {
    server
        .recv_from_server()
        .expect("Error received for welcome");
    server.send_to_server("GRAPHIC".to_string());
    thread::sleep(time::Duration::from_millis(100));
    server.send_to_server("tna".to_string());
    thread::sleep(time::Duration::from_millis(100));
    server.send_to_server("msz".to_string());
    thread::sleep(time::Duration::from_millis(100));
    server.send_to_server("mct".to_string());
    thread::sleep(time::Duration::from_millis(100));
    server.send_to_server("sgt".to_string());
    thread::sleep(time::Duration::from_millis(100));
    loop {
        match server.recv_from_server() {
            Ok(string) => {
                if !string.is_empty() {
                    {
                        let mut commands = server.commands.lock().expect("Mutex poisoned");
